  ...
  return (
    <PageWrapper className="relative bg-white min-h-screen">
      ...
      <section className="py-16 max-w-[1440px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          <div className="flex flex-col gap-3">
            ...
            <p className="text-xs sm:text-sm text-gray-500 leading-relaxed max-w-xl">
              We operate physically inside the AI Lab. Our student committee, IT volunteers, and mentor board are online on Discord during coding checkpoints.
            </p>
          </div>
          ...
          {/* Discord and Live Support Desks */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
            {/* Discord Invite Card */}
            <div className="p-6 rounded-3xl border border-primary-green/20 bg-emerald-50/15 flex flex-col justify-between gap-4">
              <div className="flex flex-col gap-1.5">
                <div className="h-8 w-8 rounded-lg bg-indigo-100 text-indigo-700 flex items-center justify-center shrink-0 text-sm font-bold">
                  D
                </div>
                <h4 className="text-sm font-extrabold text-primary-dark mt-1">Official Discord Community</h4>
                <p className="text-xs text-gray-500 leading-relaxed">
                  Join our Server to check-in with mentors, interact with team matching channels, and post GPU credits questions in dedicated support boards.
                </p>
              </div>
              <a
                href="https://discord.gg/siet-ailab-hackathon"
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center px-4 py-2.5 text-xs font-bold text-white bg-[#5865F2] hover:bg-[#4752C4] rounded-xl transition-all text-center"
              >
                Join Discord Server
              </a>
            </div>
            ...
            {/* Live Support desks */}
            <div className="p-6 rounded-3xl border border-input-border/30 bg-white flex flex-col gap-4">
              <h4 className="text-sm font-extrabold text-primary-dark flex items-center gap-2">
              ...
              <div className="flex flex-col gap-3">
                {[
                  ...
                  <div key={index} className="flex justify-between items-center text-xs border-b border-gray-100 pb-2 last:border-0 last:pb-0">
                    <div>
                      <p className="font-bold text-gray-800">{vol.name}</p>
                      <p className="text-[10px] text-gray-400 font-semibold">{vol.role}</p>
                    </div>
        ...
        <div className="rounded-3xl border border-input-border/30 bg-white p-6 sm:p-8 shadow-sm flex flex-col gap-5 h-fit">
      ...
      <section className="py-8 max-w-[1440px] mx-auto px-6 mb-16">
        ...
        <div className="relative h-72 rounded-3xl overflow-hidden border border-input-border/30 bg-card-bg shadow-sm flex flex-col items-center justify-center p-8 text-center gap-3">
          ...
          <p className="text-xs text-gray-500 max-w-md relative z-10 leading-relaxed font-semibold">
