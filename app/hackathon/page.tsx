...
type TabType = "overview" | "tracks" | "timeline" | "rules" | "judging" | "prizes" | "faq";
...
export default function HackathonPage() {
  ...
  const tabs: { id: TabType; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "judging", label: "Judging", icon: <Gavel className="h-4 w-4" /> },
    { id: "prizes", label: "Prizes", icon: <Award className="h-4 w-4" /> },
  ...
  const judgingCriteria = [
    { title: "Innovation & Originality", score: "35%", desc: "How creative, unique, and technically complex is the AI model architecture?" },
    { title: "Technical Execution", score: "35%", desc: "Is the prototype functional? Does it query APIs properly, and is it deployed publicly?" },
    { title: "Impact & Feasibility", score: "20%", desc: "Does the solution solve a real problem effectively? Can it be scaled in production?" },
    { title: "Presentation & Demo", score: "10%", desc: "Is the pitch clear, structured, and are questions answered confidently?" },
  ];
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === "overview" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                <div className="flex flex-col gap-5">
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 leading-relaxed font-medium">
                    This year, we focus on **Agentic Intelligence**—autonomous software agents that can reason, orchestrate APIs, and deliver end-to-end user flows.
                  </p>
            ...
            {activeTab === "tracks" && (
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                  {tracks.map((t, idx) => (
                    <div key={idx} className="border border-input-border/30 rounded-3xl p-6 bg-white shadow-sm flex flex-col gap-3 relative overflow-hidden group hover:border-primary-green/30 transition-all duration-300">
              ...
              <div>
                ...
                <div className="flex flex-col gap-6 max-w-3xl mx-auto">
                  {timelineSteps.map((step, idx) => {
                    const statusColors = {
                      completed: "border-primary-green bg-emerald-50 text-primary-green",
                      ongoing: "border-amber-400 bg-amber-50 text-amber-700 animate-pulse",
                      upcoming: "border-gray-200 bg-gray-50 text-gray-400"
                    };

                    return (
                      <div key={idx} className={`p-5 rounded-2xl border transition-all duration-200 flex gap-4 bg-white hover:shadow-md ${statusColors[step.status as keyof typeof statusColors].split(" ")[0]}`}>
                        <div className="flex flex-col items-center justify-center shrink-0">
                          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${statusColors[step.status as keyof typeof statusColors]}`}>
                            {step.num}
                          </span>
                        </div>
                        <div className="flex flex-col gap-1">
                          ...
                          <p className="text-[11px] sm:text-xs text-gray-500 font-medium">{step.desc}</p>
                        </div>
                      </div>
                    );
                  })}
            ...
            {activeTab === "judging" && (
              <div>
                <div className="text-center mb-10 flex flex-col items-center gap-2">
                  <h2 className="text-2xl sm:text-3xl font-extrabold text-primary-dark tracking-tight">Evaluation Rubric</h2>
                  <p className="text-xs sm:text-sm text-gray-500 font-semibold max-w-lg">Pitches are graded objectively based on pre-defined rubrics.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  {judgingCriteria.map((c, idx) => (
                    <div key={idx} className="bg-white border border-input-border/30 rounded-2xl p-5 flex flex-col gap-3 shadow-sm hover:shadow-md transition-all">
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-bold text-primary-green bg-card-bg px-2 py-1 rounded border border-input-border/10">{c.score} Weight</span>
                      </div>
                      <h4 className="text-sm sm:text-base font-extrabold text-primary-dark mt-1">{c.title}</h4>
