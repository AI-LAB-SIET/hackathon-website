...
import { Send, X, Minimize2, Trash2, Bot, User, CornerDownLeft, Sparkles, Check, Copy } from "lucide-react";
...
// Custom Markdown block/inline renderer to avoid heavy dependencies and potential security vulnerabilities
const MarkdownRenderer = React.memo(({ text }: { text: string }) => {
  ...
  // Helper to parse inline elements: **bold**, `code`, [label](href)
  const parseInline = (line: string) => {
    // Regexp for link [label](href), inline code `code`, bold **bold**
    const regex = /(\*\*.*?\*\*|`.*?`|\[.*?\]\(.*?\))/g;
  ...
  // Parse structural markdown elements: headers, lists, code blocks, paragraphs
  const parseStructure = () => {
    ...
    lines.forEach((line, idx) => {
        ...
        if (currentCodeBlock) {
          ...
          elements.push(
            <div key={blockId} className="my-3 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-950 text-slate-200 font-mono text-sm shadow-md">
              ...
              <pre className="p-4 overflow-x-auto scrollbar-none font-mono">
      ...
      if (line.startsWith("### ")) {
        flushList(idx);
        elements.push(
          <h4 key={idx} className="text-base font-bold text-slate-900 dark:text-white mt-3 mb-1.5">
      ...
      if (line.startsWith("# ")) {
        flushList(idx);
        elements.push(
          <h2 key={idx} className="text-xl font-extrabold text-slate-900 dark:text-white mt-4 mb-2">
      ...
      if (line.trim().startsWith("- ") || line.trim().startsWith("* ")) {
        ...
        listItems.push(<li key={`li-${idx}`}>{parseInline(itemContent)}</li>);
        return;
      }
      ...
      // If we were in a list and this line is not a list, flush it
      if (line.trim() === "" && inList) {
        // keep list going or flush on space
        return;
      }
...
How can I assist you with your hackathon journey today? Ask me about rules, deadlines, check-ins, or submissions!`,
  ...
  // Load contextual suggestions based on active user role
  useEffect(() => {
    const items = aiService.getSuggestions(session.role);
  ...
  useEffect(() => {
    scrollToBottom();
  }, [messages, isTyping, streamedText, scrollToBottom]);
  ...
  const handleSend = async (textToSend: string) => {
    ...
    try {
      // Send message to simulated AI service with streaming updates
      await aiService.sendMessage(textToSend, session.role, (chunk) => {
        setStreamedText(chunk);
      });

      const finalResponseText = await aiService.sendMessage(textToSend, session.role);

      const aiMsg: AIMessage = {
        id: `msg-${Date.now()}-ai`,
        sender: "ai",
        text: finalResponseText.text,
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
      };
      ...
      <div className="flex items-center justify-between px-4 py-3.5 bg-gradient-to-r from-primary-dark to-slate-900 text-white select-none">
        <div className="flex items-center gap-2.5">
          <div className="relative flex items-center justify-center w-8 h-8 rounded-full bg-primary-green/20 border border-primary-green text-primary-green">
            ...
            <span className="absolute bottom-0 right-0 w-2 h-2 rounded-full bg-primary-green border border-primary-dark animate-pulse"></span>
          </div>
          <div>
            ...
            <div className="text-[10px] text-slate-400">Online • Simulated Service</div>
          </div>
        </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-slate-50/50 dark:bg-slate-950/40">
        <AnimatePresence initial={false}>
                ...
                >
                  ...
                  <div
                    className={`text-[9px] mt-1 text-right select-none ${
                      isAI ? "text-slate-400 dark:text-slate-500" : "text-white/70"
                    }`}
                  >
                    {msg.timestamp}
                  </div>
          ...
          {isTyping && streamedText && (
            ...
            >
              <span className="flex items-center gap-1 text-[10px] text-slate-400 dark:text-slate-500 mb-1 px-1.5">
                <Bot className="w-3 h-3 text-primary-green" />
                Hack Lab Copilot
              </span>
