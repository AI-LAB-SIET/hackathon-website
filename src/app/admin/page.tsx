...
export default function AdminDashboard() {
  ...
  const {
    ...
    attendanceSlots, attendanceEntries,
  ...
  // Attendance slot states
  const [slotModalOpen, setSlotModalOpen] = useState(false);
  const [slotForm, setSlotForm] = useState({ name: "", description: "", scheduledAt: "", isActive: false });
          ...
          >
            ...
            {activeTab === "attendance" && (() => {
              ...
              const exportCSV = () => {
                ...
                toast("Exported as CSV (opens in Excel)", "success");
              };

              const handleCreateSlot = async () => {
                if (!slotForm.name.trim()) { toast("Slot name is required", "error"); return; }
                if (!activeHackathonId) { toast("Select an active hackathon first", "error"); return; }
                setSlotSaving(true);
                try {
                  const id = await createAttendanceSlot({ hackathonId: activeHackathonId, name: slotForm.name.trim(), description: slotForm.description.trim(), scheduledAt: slotForm.scheduledAt || new Date().toISOString(), isActive: slotForm.isActive, createdBy: session.email || "" });
                  if (slotForm.isActive) {
                    for (const slot of attendanceSlots) {
                      if (slot.isActive) {
                        await updateAttendanceSlot(slot.id, { isActive: false });
                      }
                    }
                  }
                  setSelectedSlotId(id);
                  setSlotModalOpen(false);
                  setSlotForm({ name: "", description: "", scheduledAt: "", isActive: false });
                  toast("Attendance slot created!", "success");
                } catch { toast("Failed to create slot", "error"); } finally { setSlotSaving(false); }
              };
              ...
              return (
                <div className="flex flex-col gap-6">
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div>
                      <h3 className="text-base font-bold text-primary-dark dark:text-gray-100">Attendance Slots</h3>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Create and manage attendance sessions. Staff mark participants present per slot.</p>
                    </div>
                    <Button size="sm" className="bg-primary-green hover:bg-emerald-600 text-white gap-1.5" onClick={() => setSlotModalOpen(true)}>
                      <Plus className="h-3.5 w-3.5" /> New Slot
                    </Button>
                  </div>

                  {attendanceSlots.length === 0 ? (
                    <div className="py-12 text-center rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 text-gray-400 text-sm">
                      No attendance slots yet. Click <strong>New Slot</strong> to create one.
                    </div>
                  ) : (
                    <div className="flex flex-wrap gap-2">
                      {attendanceSlots.map(slot => (
                        <button key={slot.id} onClick={() => setSelectedSlotId(slot.id)}
                          className={`px-3.5 py-1.5 rounded-full text-xs font-bold border transition-all ${selectedSlotId === slot.id ? "bg-primary-green text-white border-emerald-600 shadow-sm" : "bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700 hover:border-primary-green"}`}>
                          {slot.isActive && <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-400 mr-1.5 align-middle" />}
                          {slot.name}
                  ...
                  {selectedSlotId && activeSlot && (
                    <div className="rounded-3xl border border-gray-150 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-hidden">
                      <div className="p-5 border-b border-gray-100 dark:border-gray-800 flex items-start justify-between gap-3">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {activeSlot.isActive && <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 uppercase">Active</span>}
                          </div>
                          {activeSlot.description && <p className="text-xs text-gray-500 mt-0.5">{activeSlot.description}</p>}
                          <p className="text-[11px] text-gray-400 mt-1">
                            Scheduled: {activeSlot.scheduledAt ? new Date(activeSlot.scheduledAt).toLocaleString() : "—"} · {slotEntries.length} present / {allParticipants.length} total
                          </p>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <Button size="sm" variant="outline"
                            className={`h-7 text-[10px] ${activeSlot.isActive ? "text-amber-600 hover:bg-amber-50" : "text-emerald-600 hover:bg-emerald-50"}`}
                            onClick={async () => {
                              const newStatus = !activeSlot.isActive;
                              await updateAttendanceSlot(activeSlot.id, { isActive: newStatus });
                              if (newStatus) {
                                for (const slot of attendanceSlots) {
                                  if (slot.id !== activeSlot.id && slot.isActive) {
                                    await updateAttendanceSlot(slot.id, { isActive: false });
                                  }
                                }
                              }
                            }}>
                            {activeSlot.isActive ? "Deactivate" : "Activate"}
                          </Button>
                          <Button size="sm" variant="outline" className="h-7 text-[10px] text-red-500 hover:bg-red-50"
                            onClick={async () => { if (confirm(`Delete slot "${activeSlot.name}" and all its records?`)) { await deleteAttendanceSlot(activeSlot.id); setSelectedSlotId(null); toast("Slot deleted","success"); } }}>
                      ...
                      <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center gap-3 flex-wrap">
                        ...
                        <Button size="sm" className="h-7 text-[10px] bg-primary-green/10 text-emerald-700 hover:bg-primary-green hover:text-white dark:bg-emerald-950/30 dark:text-emerald-400"
                          onClick={async () => { for (const t of approvedTeams) await bulkMarkTeamAttendance(selectedSlotId, t.id); toast("All teams marked present!","success"); }}>
                          Mark All Present
                        </Button>
                      </div>
                      <div className="overflow-x-auto">
                        <table className="w-full text-xs text-left">
                          <thead className="bg-gray-50 dark:bg-gray-800/50">
                            <tr className="text-gray-500 dark:text-gray-400">
                              <th className="px-4 py-3 font-bold uppercase text-[10px] tracking-wider">#</th>
                              <th className="px-4 py-3 font-bold uppercase text-[10px] tracking-wider">Participant</th>
                              <th className="px-4 py-3 font-bold uppercase text-[10px] tracking-wider">Reg. No.</th>
                              <th className="px-4 py-3 font-bold uppercase text-[10px] tracking-wider">Team</th>
                              <th className="px-4 py-3 font-bold uppercase text-[10px] tracking-wider">Status</th>
                              <th className="px-4 py-3 font-bold uppercase text-[10px] tracking-wider">Marked At</th>
                              <th className="px-4 py-3 font-bold uppercase text-[10px] tracking-wider">Action</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                              ...
                              return (
                                <tr key={p.email} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/20 transition-colors">
                                  ...
                                  <td className="px-4 py-3">
                                    ...
                                    <div className="text-[10px] text-gray-400">{p.department} · {p.year} · {p.hostelStatus||"—"}</div>
                                  </td>
                                  <td className="px-4 py-3 font-mono text-gray-600 dark:text-gray-400">{p.registerNumber||"—"}</td>
                                  <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-md bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium">{p.teamName}</span></td>
                                  <td className="px-4 py-3">
                                    {entry ? <span className="px-2.5 py-1 rounded-full bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 font-bold">Present</span>
                                           : <span className="px-2.5 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 font-bold">Absent</span>}
                                  </td>
                                  <td className="px-4 py-3 text-gray-500">{entry ? new Date(entry.markedAt).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}) : "—"}</td>
                                  <td className="px-4 py-3">
                                    {entry
                            ...
                            {filteredParticipants.length === 0 && (
                              <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400 italic">No participants found.</td></tr>
                            )}
                          </tbody>
                        </table>
                      </div>
                  ...
                  <Modal isOpen={slotModalOpen} onClose={() => setSlotModalOpen(false)} title="Create Attendance Slot">
                    <div className="flex flex-col gap-4 p-1">
                      <div>
                        <label className="text-xs font-bold text-gray-600 dark:text-gray-400 block mb-1">Slot Name *</label>
                        <input type="text" placeholder="e.g. Day 1 — Morning Session" value={slotForm.name} onChange={e => setSlotForm(p => ({...p, name: e.target.value}))}
                          className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-green/30" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-gray-600 dark:text-gray-400 block mb-1">Description</label>
                        <input type="text" placeholder="Optional description" value={slotForm.description} onChange={e => setSlotForm(p => ({...p, description: e.target.value}))}
                          className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-green/30" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-gray-600 dark:text-gray-400 block mb-1">Scheduled Date & Time</label>
                        <input type="datetime-local" value={slotForm.scheduledAt} onChange={e => setSlotForm(p => ({...p, scheduledAt: e.target.value}))}
                          className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-green/30" />
                      </div>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={slotForm.isActive} onChange={e => setSlotForm(p => ({...p, isActive: e.target.checked}))} className="rounded border-gray-300 text-primary-green" />
                        <span className="text-sm text-gray-700 dark:text-gray-300">Mark as Active Slot</span>
                      </label>
                      <div className="flex gap-2 pt-2">
                        <Button onClick={handleCreateSlot} isLoading={slotSaving} className="flex-1 bg-primary-green hover:bg-emerald-600 text-white">Create Slot</Button>
                        <Button variant="outline" onClick={() => setSlotModalOpen(false)} className="flex-1">Cancel</Button>
                      </div>
                    </div>
                  </Modal>
                </div>
